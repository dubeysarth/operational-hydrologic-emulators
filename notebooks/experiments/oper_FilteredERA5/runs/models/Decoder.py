...
class Decoder(nn.Module):
    ...
    def forward(self, sample, available, enc_h_lag, enc_c_lag):
        ...
        # X_dynamic = sample['X_dynamic_HRES'].to(self.device) # (lead, batch_size, dim_in_embedding_lead_dynamic + 4)
        X_dynamic = sample['X_dynamic_ERA5_lead'].to(self.device) # (lead, batch_size, dim_in_embedding_lead_dynamic + 4)
        indices = [0, 1, 4, 8, 9]
        first_features = X_dynamic[:, :, indices]
        last_features = X_dynamic[:, :, -4:]
        X_dynamic = torch.cat([first_features, last_features], dim=-1)
